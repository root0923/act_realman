#!/usr/bin/env python3
# filepath: /home/shuxiangzhang/act-yang/真机控制_realtime.py
...
import copy
class Listener():
    def __init__(self, local_ip, remote_ip, port,recv_time=2):
        # 假定recv_time是int
        assert isinstance(recv_time, int), "recv_time must be an integer"
        ...
        while True:
            try:
                ...
                print(f"尝试绑定UDP端口: {self.local_ip}:{self.port}")
                self.socket.bind((self.local_ip, self.port))
                print(f"绑定UDP端口成功: {self.local_ip}:{self.port}")
                break
            except Exception as e:
                logging.error(f"绑定UDP端口失败: {e}")
                self.socket.close()  # 关闭之前的socket
                self.port += 1  # 尝试下一个端口
                time.sleep(0.5)
...
#             return hand_status['hand']['hand_pos']
#         else:
...
#         print(f"获取灵巧手状态出错: {e}")
#         return [0, 0, 0, 0, 0, 0]  # 默认值
def get_current_hand_state(listener_instance: Listener):
    """
    获取额外两个关节的角度。
...
def main(args):
    ...
    init_pose = [-65, 30, -80, 13, -69, -246]  # 主臂从臂的初始位姿
    ...
    # 确保机械臂已经到达初始位置，等待用户按下g键开始推理
    print("\n========================================")
    print("机械臂已移动到初始位置")
    print("按下 'g' 键然后按 Enter 键开始推理和控制...")
    print("========================================")
    ...
    if temporal_agg:
        ...
        all_time_actions = torch.zeros([1, 10000 + num_queries, action_dim]).cuda()  # 假设最多1000帧
    ...
    listener_instance = Listener(
        local_ip=args.get('local_ip', "192.168.110.118"),
        remote_ip=args.get('robot_ip', "192.168.110.118"),
        port=args.get('udp_port', 8083),
        recv_time=args.get('udp_recv_time',2)  # 接收时间间隔，单位为秒
    )
    listener_instance.start(master_arm, config=None)
    t1 = threading.Thread(target=listener_instance.receive_data)
    t1.daemon = True  # 设置为守护线程，主线程结束时自动结束
    t1.start()
    
    print("开始实时控制循环。按'q'键退出。")
    try:
        with torch.inference_mode():
            while True:
                ...
                hand_state = get_current_hand_state(listener_instance)
                ...
                # 9. 发送命令到机械臂和灵巧手
                # 如果启用execute_actions，则发送动作
                if args.get('execute_actions', True):  # 默认启用控制
                    # 控制机械臂 - 使用CAN FD通信以获得更快的响应
                    arm_result = master_arm.rm_movej_canfd(arm_action.tolist(), False, 0, 1, 60)
                    if arm_result != 0:
        ...
        # 关闭UDP监听
        try:
            listener_instance.close()
            print("UDP监听已关闭")
        except Exception as e:
            print(f"关闭UDP监听时出现异常: {e}")
        
        # 等待用户输入 'g' 键后再复位机械臂和灵巧手
        print("推理已停止。按下 'g' 键然后回车以复位机械臂和灵巧手...")
        while True:
            user_input = input()
            if user_input.lower() == 'g':
                print("开始复位机械臂和灵巧手...")
                ...
                print("请输入 'g' 然后按 Enter 键以复位...")

        # 将机械臂安全移回初始位置
        print("正在将机械臂安全移回初始位置...")
        try:
            # 使用较慢的速度确保安全
            slow_speed = 15  # 降低速度百分比
            result = master_arm.rm_movej(init_pose, slow_speed, 0, 0, 1)
            
            if result != 0:
                print(f"警告：移回初始位置失败，错误码: {result}，尝试再次移动...")
                # 再次尝试，使用更保守的参数
                result = master_arm.rm_movej(init_pose, 10, 0, 0, 1)
                
                if result != 0:
                    print(f"再次移动失败，错误码: {result}")
                else:
                    print("第二次尝试成功，机械臂已安全回到初始位置")
            else:
                print("机械臂已安全回到初始位置")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--execute_actions', action='store_true', default=True, help='是否执行动作控制机械臂')
    ...
    parser.add_argument('--control_fps', default=30, type=int, help='控制循环频率')
    
    # UDP监听参数
    parser.add_argument('--local_ip', default='192.168.110.164', type=str, help='本地IP地址(用于获得UDP数据报)')
    parser.add_argument('--udp_port', default=8083, type=int, help='UDP端口(用于接收灵巧手数据)')
    parser.add_argument('--udp_recv_time', default=2, type=int, help='UDP接收时间间隔(k*5ms)')
