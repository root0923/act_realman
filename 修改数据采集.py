...
class Listener():
    def __init__(self, local_ip, remote_ip, port=8085, recv_time=2, max_retries=10):
        ...
        self.hand_pos_history = []  # 用于存储手部位置的历史记录
        # 尝试绑定端口，如果失败则尝试其他端口
        retry_count = 0
    ...
    def close(self):
        ...
        time.sleep(0.5)
        self.socket.close()
        self.hand_pos_history = []  # 清空手部位置历史记录
...
def listener(shared_array, event,now_pos):
    ...
    while True:
        if event.wait(1):  # 等待事件触发（最多0.1秒）
            ...
            # 时间平滑
            if len(hand_pos_history) >= 10:
                hand_pos_history.pop(0)
            hand_pos_history.append(hand_pos)
            hand_pos = np.mean(hand_pos_history, axis=0).astype(int).tolist()
            robot.rm_set_hand_follow_pos(hand_pos, 0)
        else:
            if len(hand_pos_history) > 1:
                # 如果没有新的数据更新，使用历史数据
                hand_pos_history.pop(0)  # 移除最旧的记录
                hand_pos = np.mean(hand_pos_history, axis=0).astype(int).tolist()
                robot.rm_set_hand_follow_pos(hand_pos, 0)
...
def get_extra_joints_angles(listener_instance):
    ...
    hand_pos = data['hand']['hand_pos'][:]        
    listener_instance.hand_pos_history.append(hand_pos)
    if len(listener_instance.hand_pos_history) > 8:
        listener_instance.hand_pos_history.pop(0)
    weight = [0.05,0.05,0.1,0.1,0.1,0.15,0.2,0.25]
    hand_pos = np.array(listener_instance.hand_pos_history)
    # 加权平均手部位置
    hand_pos = np.average(hand_pos, axis=0).tolist()
    return hand_pos
...
if __name__ == "__main__":
    ...
    for episode in range(1, num_episodes + 1):
        ...
        # 保存当前组数据
        data_path = os.path.join(os.getcwd(), 'data_fliter', f'b{episode}')
        hdf5datapath = os.path.join(os.getcwd(), 'data_fliter', 'data')
