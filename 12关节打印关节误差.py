...
def main(args):
    ...
    with torch.inference_mode():
        for ak in range(1, 20):
            datapath = f'/home/shuxiangzhang/act_realman/data_12_grasp/data/real_test01/episode_{ak}.hdf5'
            ...
            plt.title('前六个关节预测误差曲线')
            plt.legend(loc='best')
            plt.grid(True)
            plt.tight_layout()
            save_dir = '/home/shuxiangzhang/act_realman/ckpt/error/error_time'
            os.makedirs(save_dir, exist_ok=True)
            save_path = f'{save_dir}/prediction_joint_error_前六_{ak}.png'
            plt.savefig(save_path)
            plt.close()
            print(f'前六关节误差曲线已保存: {save_path}')
            ...
            plt.xlabel('Timestep')
            plt.ylabel('Joint Error (Prediction - Ground Truth)')
            plt.title('后六个关节预测误差曲线')
            plt.legend(loc='best')
            plt.grid(True)
            plt.tight_layout()
            save_path = f'{save_dir}/prediction_joint_error_后六_{ak}.png'
            plt.savefig(save_path)
            plt.close()
            print(f'后六关节误差曲线已保存: {save_path}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_dir', default='/home/shuxiangzhang/act-realman/ckpt/real_test_grasp_process', type=str, help='ckpt_dir', required=False)
