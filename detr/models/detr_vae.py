...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, vq, vq_class, vq_dim, action_dim):
        ...
        # print('state_dim:', state_dim, 'hidden_dim:', hidden_dim, 'action_dim:', action_dim)
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, vq_sample=None):
        ...
        if self.backbones is not None:
            ...
            for cam_id, cam_name in enumerate(self.camera_names):
                ...
                # 对于单摄像头，直接使用第一个键
                # if isinstance(cam_id, slice):
                #     cam_key = list(self.backbones.keys())[0]
                #     features, pos = self.backbones[cam_key](image)
                # else:
                #     features, pos = self.backbones[cam_id](image[:, cam_id])
...
def build(args):
    #state_dim = 14 # TODO hardcode不要硬编码
    state_dim = args.state_dim  # 从参数获取
