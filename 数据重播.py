...
def replay_episode(h5_path, wait_ms=33, save_video=True):
    with h5py.File(h5_path, 'r') as f:
        cam01 = f['observations/images/cam01'][:]
        cam02 = f['observations/images/cam02'][:]
        actions = f['action'][:]
        ...
        action_lines_2 = [ax[0,1].plot([], [], label=f'Action {i+7}')[0] for i in range(6)]
        qpos_lines_1 = [ax[1,0].plot([], [], label=f'Qpos {i+1}')[0] for i in range(6)]
        qpos_lines_2 = [ax[1,1].plot([], [], label=f'Qpos {i+7}')[0] for i in range(6)]
        ax[0,0].set_title('Action 前六关节')
        ax[0,1].set_title('Action 后六关节')
        ax[1,0].set_title('Qpos 前六关节')
        ax[1,1].set_title('Qpos 后六关节')
        for a in ax.flatten():
            a.set_xlim(0, n)
        ...
        for i in range(n):
            ...
            for j, line in enumerate(action_lines_2):
                line.set_data(x, action_hist[:, j+6])
            for j, line in enumerate(qpos_lines_2):
                line.set_data(x, qpos_hist[:, j+6])
            for a in ax.flatten():
                a.set_xlim(0, n)
            plt.pause(0.001)
...
if __name__ == "__main__":
    files = sorted(glob.glob('/home/shuxiangzhang/act_realman/data_handchange/data/real_test01/episode_*.hdf5'))
    print("可用文件：")
