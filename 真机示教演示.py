...
class Listener():
    ...
    def start(self,arm,config=None):
        ...
        config.joint_speed = 1
        config.lift_state = 1
        config.expand_state = 0
        config.hand_state = 1
        config.arm_current_status = 1
        config.aloha_state = 1
        self.config = config
        config_setting = rm_realtime_push_config_t(self.recv_time, True, self.port, 0, self.local_ip, config)
        self.is_close = False
        print(arm.rm_set_realtime_push(config_setting),"设置实时推送配置成功")
        self.config_setting = arm.rm_get_realtime_push()
        time.sleep(0.5)  # 确保配置生效
        print("当前实时推送配置:", self.config_setting)
        print("计划配置:", config_setting.to_dict())
...
def main(args):
    ...
    listener_instance = Listener(
        local_ip=args.get('local_ip', "192.168.110.164"),
        remote_ip=args.get('robot_ip', "192.168.110.118"),
        port=args.get('udp_port', 8082),
    ...
    # config.aloha_state = 1
    config = None
    listener_instance.start(arm=master_arm,config=config)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--udp_port', default=8100, type=int, help='UDP端口(用于接收灵巧手数据)')
