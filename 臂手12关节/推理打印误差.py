...
def main(args):
    ...
    nheads = 8
    # 修改为12维度
    state_dim = task_config.get('state_dim', 12)
    action_dim = task_config.get('action_dim', 12)
    policy_config = {
        'lr': 1e-5,
    ...
    with torch.inference_mode():
        for ak in range(1, 3):
            datapath = f'/home/shuxiangzhang/act-yang/data/real_test01/episode_{ak}.hdf5'
            ...
            # 处理数据维度不匹配的情况
            print(f'Original shapes - Qpos: {all_qpos.shape}, Actions: {all_actions.shape}')
            
            # 如果输入数据维度小于模型期望的维度，进行填充
            if all_qpos.shape[1] < state_dim:
                pad_width = state_dim - all_qpos.shape[1]
                all_qpos = np.pad(all_qpos, ((0, 0), (0, pad_width)), 'constant')
                print(f'Padded qpos shape: {all_qpos.shape}')
            
            if all_actions.shape[1] < action_dim:
                pad_width = action_dim - all_actions.shape[1]
                all_actions = np.pad(all_actions, ((0, 0), (0, pad_width)), 'constant')
                print(f'Padded actions shape: {all_actions.shape}')
            
            # 如果输入数据维度大于模型期望的维度，进行截断
            if all_qpos.shape[1] > state_dim:
                all_qpos = all_qpos[:, :state_dim]
                print(f'Truncated qpos shape: {all_qpos.shape}')
            
            if all_actions.shape[1] > action_dim:
                all_actions = all_actions[:, :action_dim]
                print(f'Truncated actions shape: {all_actions.shape}')
            ...
            for t in tqdm(range(length), desc=f'Processing timesteps {ak}/{length}'):
            ...
            # 绘制每个关节的误差曲线，为了清晰可分，将12个关节分成两组绘制
            # 第一组关节 (1-6)
            plt.figure(figsize=(14, 8))
            x = np.arange(length)
            for i in range(min(6, errors.shape[1])):
                plt.plot(x, errors[:, i], label=f'Joint {i+1}')
            plt.xlabel('Timestep')
            plt.ylabel('Joint Error (Prediction - Ground Truth)')
            plt.title('Per-joint Prediction Error Curve (Joints 1-6)')
            plt.legend(loc='best')
            plt.grid(True)
            plt.tight_layout()

            # 保存到指定路径
            save_path = f'/home/shuxiangzhang/act-yang/ckpt/error/error_time/prediction_joint_error_{ak}_group1.png'
            plt.savefig(save_path)
            print(f'Error curve for joints 1-6 saved to {save_path}')
            
            # 第二组关节 (7-12)
            if errors.shape[1] > 6:
                plt.figure(figsize=(14, 8))
                for i in range(6, errors.shape[1]):
                    plt.plot(x, errors[:, i], label=f'Joint {i+1}')
                plt.xlabel('Timestep')
                plt.ylabel('Joint Error (Prediction - Ground Truth)')
                plt.title('Per-joint Prediction Error Curve (Joints 7-12)')
                plt.legend(loc='best')
                plt.grid(True)
                plt.tight_layout()

                # 保存到指定路径
                save_path = f'/home/shuxiangzhang/act-yang/ckpt/error/error_time/prediction_joint_error_{ak}_group2.png'
                plt.savefig(save_path)
                print(f'Error curve for joints 7-12 saved to {save_path}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_dir', default='/home/shuxiangzhang/act-yang/ckpt/real_test01', type=str, help='ckpt_dir', required=False)
